...
- *Cmk 与 Cpk 的关系*：
  - Cmk 不考虑子组分割，而是使用极短时间内连续取样的整体标准差进行计算。
  - Cmk 要求在受控时间内连续取样（避免长期漂移），而 Cpk 使用子组内极差的 $d_2$ 变换来隔离长期的组间漂移。
...
*适用条件*：
- *计算 Cmk*：推荐采集 50-100 个连续样品（单机调试，不分组，受控时间内）
- *计算 Ppk*：全量长期数据（包含所有时间漂移，不考虑子组分割）
...
*关键差异*：
- *Cmk* 侧重于评估设备在极短时间内的硬件重复精度（利用短时间整体 $sigma$）
